...
async function checkForMutant(req: Request, res: Response) {
	try {
		...
		if (isMutantResult) {
			...
			await Stats.updateOne({}, { $inc: { count_mutant_dna: 1 } }, { upsert: true });
		} else {
			await DnaSequence.create({ dna, isMutant: false });
			await Stats.updateOne({}, { $inc: { count_human_dna: 1 } }, { upsert: true });
		}

		const stats = await Stats.findOne();
		if (stats) {
			const ratio = stats.count_human_dna === 0 ? 1 : stats.count_mutant_dna / stats.count_human_dna;
